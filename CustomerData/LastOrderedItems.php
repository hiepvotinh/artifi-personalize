 ...
 */
namespace Artifi\Personalize\CustomerData;
        ...
        if ($order) {
            ...
            foreach ($order->getParentItemsRandomCollection($limit) as $item) {
                if ($item->getProduct() && in_array($website, $item->getProduct()->getWebsiteIds())) {
                    ...
                    $productFull = $this->productRepository->getById($productId);
                    $items[] = [
                        'id' => $item->getId(),
                        'name' => $item->getName(),
                        'url' => $item->getProduct()->getProductUrl(),
                        'is_saleable' => $this->isItemAvailableForReorder($item),
                        'personalization_allowed' => (int)$productFull->getPersonalizationAllowed(),
                        'personalization_required' => (int)$productFull->getPersonalizationRequired()
                    ];
                }
            }
        }
